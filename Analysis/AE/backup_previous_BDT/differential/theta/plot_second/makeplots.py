...
stackList = {"TTToSemiLeptonic_1":[kRed], "TTToSemiLeptonic_2":[kRed], "wjetstolnu":[kGreen], "dyjetstoll_m-50_ht":[kYellow], "st":[kBlue], "qcd_ht":[kOrange], "diboson":[kTeal], "zgammatojjgamma":[kCyan+2], "wgtolnug":[kViolet], "gjets_int":[kRed-2]}
...
sample_names = ["wjetstolnu", "st", "dyjetstoll_m-50_ht", "qcd_ht","TTToSemiLeptonic_1","TTToSemiLeptonic_2"]
...
for sample in sample_names:
    ...
    for i in range(1,total_n_bins+1):
        if(i < 3):              h1.SetBinContent(i,hist1_[sample].GetBinContent(i))
        if(i >=3 and i < 4):   h1.SetBinContent(i,hist2_[sample].GetBinContent(i - 2))
        if(i==4):              h1.SetBinContent(i,hist2_[sample].GetBinContent(2))
        if(i > 4 and i < 6):  h1.SetBinContent(i,hist3_[sample].GetBinContent(i - 4))
        if(i==6):              h1.SetBinContent(i,hist3_[sample].GetBinContent(2))
...
for i in range(1,total_n_bins+1):
    if(i < 3):              data.SetBinContent(i,dataHist1.GetBinContent(i))
    if(i >=3 and i < 4):   data.SetBinContent(i,dataHist2.GetBinContent(i - 2))
    if(i==4):              data.SetBinContent(i,dataHist2.GetBinContent(2))
    if(i > 4 and i < 6):  data.SetBinContent(i,dataHist3.GetBinContent(i - 4))
    if(i==6):              data.SetBinContent(i,dataHist3.GetBinContent(2))
...
l1 = TLine(2.0,0,2.0,20000)
l1.Draw("same")

l2 = TLine(4,0,4,20000)
l2.Draw("same")

latex = TLatex();
latex.SetTextSize(0.05);
latex.SetTextAlign(13);
latex.DrawLatex(0.02,18**(1.66*log10(maxVal) - 0.5*log10(minVal)),"CMS #font[12]{preliminary}");
latex.DrawLatex(4.0,18**(1.67*log10(maxVal) - 0.5*log10(minVal)),"138 fb^{-1} (13 TeV)");

latex.SetTextSize(0.03)
latex.DrawLatex(0.5,50000,"#theta_{j} #in [0,0.25#pi]");
latex.DrawLatex(2.5,50000,"#theta_{j} #in [0.25#pi,0.75#pi]");
latex.DrawLatex(4.5,50000,"#theta_{j} #in [0.75#pi,#pi]");
...
for i in range(1,total_n_bins+1):
    ...
    if(i < 3):              errorband.SetBinError(i,data_bin1.GetBinError(i)/bkg_bin1.GetBinContent(i))
    if(i >=3 and i < 4):   errorband.SetBinError(i,data_bin2.GetBinError(i - 2)/bkg_bin2.GetBinContent(i-2))
    if(i==4):              errorband.SetBinError(i,data_bin2.GetBinError(2)/bkg_bin2.GetBinContent(2))
    if(i > 4 and i < 6):  errorband.SetBinError(i,data_bin3.GetBinError(i - 4)/bkg_bin3.GetBinContent(i-4))
    if(i==6):              errorband.SetBinError(i,data_bin3.GetBinError(2)/bkg_bin3.GetBinContent(2))
...
for i in range(1,total_n_bins+1):
    if(i < 3):              errorband2.SetBinError(i,4*math.sqrt(data_bin1.GetBinError(i)**2 + bkg_bin1.GetBinError(i)**2) /bkg_bin1.GetBinContent(i))
    if(i >=3 and i < 4):   errorband2.SetBinError(i,4*math.sqrt(data_bin2.GetBinError(i-2)**2+bkg_bin2.GetBinError(i-2)**2)/bkg_bin2.GetBinContent(i-2))
    if(i==4):              errorband2.SetBinError(i,4*math.sqrt(data_bin2.GetBinError(2)**2+bkg_bin2.GetBinError(2)**2)/bkg_bin2.GetBinContent(2))
    if(i > 4 and i < 6):  errorband2.SetBinError(i,4*math.sqrt(data_bin3.GetBinError(i-4)**2+bkg_bin3.GetBinError(i-4)**2)/bkg_bin3.GetBinContent(i-4))
    if(i==6):              errorband2.SetBinError(i,4*math.sqrt(data_bin3.GetBinError(2)**2+bkg_bin3.GetBinError(2)**2)/bkg_bin3.GetBinContent(2))
