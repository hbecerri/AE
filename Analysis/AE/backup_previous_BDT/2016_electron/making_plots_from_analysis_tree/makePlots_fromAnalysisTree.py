...
_fileDir ="/data/dust/user/hugobg/analysis/electron"
...
#stackList = { "TTToSemiLeptonic_2016preVFP_2":[kRed]} 
#stackList = {"TTToHadronic_2016preVFP":[kRed-3], "TTToOthers":[kRed+2], "TTToSemiLeptonic_2016preVFP":[kRed],"DYJetsToLL_M-50_HT_2016preVFP":[kBlue], "QCD_HT_2016preVFP":[kTeal], "WJetsToLNu_2016preVFP":[kGreen], "ST_2016preVFP":[kYellow], "WW_WZ_ZZ_2016preVFPv3":[kOrange]}
stackList = { "TTToOthers":[kRed+2],"TTToSemiLeptonic_2016preVFP":[kRed], "Others":[kBlue], "WJetsToLNu_2016preVFP":[kGreen]}
...
sample_names = ["TTTo2L2Nu_2016preVFP","TTToHadronic_2016preVFP","DYJetsToLL_M-50_HT_2016preVFP","ST_2016preVFP","WJetsToLNu_2016preVFP","TTToSemiLeptonic_2016preVFP"]
...
for sample in sample_names:
		...
		tree_MC[sample]=_file[sample].Get("AnalysisTree")
 	        ...
 	        tree_MC[sample].Draw("%s>>h2_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"eventweight*(N_Ak4 >=3 && Ele_pt > 55.0 && DeepttagN <= 1 && btagN >= 1 && M_tt > 450 &&chi2 < 30 && Ak4_add_pt > 100 && angle_theta_opt > 0.25*3.1459 && angle_theta_opt < 0.55*3.1459)")
                hist1_[sample] = tree_MC[sample].GetHistogram()

                if(sample == "DYJetsToLL_M-50_HT_2016preVFP" or sample == "QCD_HT_2016preVFP" or sample == "ST_2016preVFP"):
                    hist1_[sample].SetFillColor(stackList["Others"][0])
                    hist1_[sample].SetLineColor(stackList["Others"][0])
                    if(sample == "ST_2016preVFP"):
                        legendR.AddEntry(hist1_[sample],"Others",'f')

                else:
                    if(sample == "TTToSemiLeptonic_2016preVFP"):
                    ...
                    if(sample == "WJetsToLNu_2016preVFP"):
...
_file["Data"] = TFile("%s/uhh2.AnalysisModuleRunner.DATA.DATA_SingleMuon_Run2016preVFP.root"%(_fileDir),"read")
print "%s/uhh2.AnalysisModuleRunner.DATA.DATA.root"%(_fileDir)

tree = _file["Data"].Get("AnalysisTree")
tree.Draw("%s>>dat_hist(%i,%i,%f)"%(histName,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"1*(N_Ak4 >=3 && Ele_pt > 55.0 && DeepttagN <= 1 && btagN >= 1 && M_tt > 450 &&chi2 < 30 && Ak4_add_pt > 100 && angle_theta_opt > 0.25*3.1459 && angle_theta_opt < 0.55*3.1459)")
