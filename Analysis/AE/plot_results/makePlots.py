...
for sample in sample_names:
    ...
    hist1_[sample] = _file[sample].Get("muon_2018_750_900_prefit/%s"%sample)
    hist1_2017[sample] = _file_2017[sample].Get("muon_2017_750_900_prefit/%s"%sample)
    hist1_2016[sample] = _file_2016[sample].Get("muon_2016postVFP_750_900_prefit/%s"%sample)
    hist1_2016pre[sample] = _file_2016pre[sample].Get("muon_2016postVFP_750_900_prefit/%s"%sample)
...
dataHist1=data1.Get("muon_2018_750_900_prefit/data_obs")
dataHist1_2017=data2.Get("muon_2017_750_900_prefit/data_obs")
dataHist1_2016=data3.Get("muon_2016postVFP_750_900_prefit/data_obs")
dataHist1_2016pre=data4.Get("muon_2016postVFP_750_900_prefit/data_obs")
...
for i in range(1, h_ratio.GetNbinsX() + 1):
    h_data = data.GetBinContent(i)
    h_mc = stack.GetBinContent(i)
 
    if mc == 0:
        continue  # Avoid division by zero
 
    data_error = data.GetBinError(i)
    mc_error = stack.GetBinError(i)  # Statistical error
 
    # Assume a systematic uncertainty (as an example)
    mc_syst_error = 0.0 * mc  # 10% systematic uncertainty
 
    # Total MC error
    mc_total_error = add_in_quadrature(mc_error, mc_syst_error)
 
    # Error on the ratio
    ratio_error = (h_data / h_mc) * add_in_quadrature(data_error / h_data, mc_total_error / h_mc)
    h_errorband.SetBinError(i, ratio_error)
    h_errorband2.SetBinError(i,(h_data / h_mc)*(data_error / h_data))
