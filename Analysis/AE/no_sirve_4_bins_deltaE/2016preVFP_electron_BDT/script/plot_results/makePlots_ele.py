...
sample_names = ["diboson", "dyjetstoll_m-50_ht", "qcd_ht", "st", "wjetstolnu","ttto2l2nu","tttosemileptonic_outfiducial","TTToSemiLeptonic_1","TTToSemiLeptonic_2"]
...
for sample in sample_names:
    ...
    _file_2017[sample]  = TFile("shapes.root","read")
    _file_2016[sample]  = TFile("shapes.root","read")
    _file_2016pre[sample]  = TFile("shapes.root","read")

    hist1_[sample] = _file[sample].Get("electron_2018_750_900_prefit/%s"%sample)
    hist1_2017[sample] = _file_2017[sample].Get("electron_2017_750_900_prefit/%s"%sample)
    hist1_2016[sample] = _file_2016[sample].Get("electron_2016postVFP_750_900_prefit/%s"%sample)
    hist1_2016pre[sample] = _file_2016pre[sample].Get("electron_2016postVFP_750_900_prefit/%s"%sample)

    total_n_bins = hist1_[sample].GetXaxis().GetNbins()
    h1 = TH1F("h1","test1",nb,array('d',Bin))
    for i in range(1,total_n_bins+1):
        if(i < 100):              h1.SetBinContent(i,hist1_[sample].GetBinContent(i))
    ...
    h4 = TH1F("h4","test2",nb,array('d',Bin))
    for i in range(1,total_n_bins+1):
        if(i < 100):              h4.SetBinContent(i,hist1_2016pre[sample].GetBinContent(i))
    h4.SetFillColor(stackList[sample][0])
    h4.SetLineColor(stackList[sample][0])

    if(sample == 'TTToSemiLeptonic_1') : legendR.AddEntry(h1,'t#bar{t} (l+jets)','f')
    if(sample == 'diboson') : legendR.AddEntry(h1,'Others','f')
    if(sample == 'wjetstolnu') : legendR.AddEntry(h1,'W + jets','f')
    if(sample == 'ttgjets') : legendR.AddEntry(h1,'t#bar{t} #gamma','f')
    if(sample == 'diphoton') : legendR.AddEntry(h1,'#gamma#gamma','f')
    if(sample == 'fake') : legendR.AddEntry(h1,'fake #gamma','f')
    if(sample == 'wgammatojjgamma') : legendR.AddEntry(h1,'W(jj) #gamma','f')
    if(sample == 'zgammatojjgamma') : legendR.AddEntry(h1,'Z(jj) #gamma','f')
    if(sample == 'wgtolnug') : legendR.AddEntry(h1,'W(l#nu) #gamma','f')
    if(sample == 'tttosemileptonic_outfiducial') : legendR.AddEntry(h1,'t#bar{t} (out)','f')
    if(sample == 'ttto2l2nu') : legendR.AddEntry(h1,'t#bar{t} (others)','f')

    stack.Add(h1)
    stack.Add(h2)
    stack.Add(h3)
    stack.Add(h4)
    del h1; del h2; del h3; del h4


data1 = TFile("shapes.root","read")
data2 = TFile("shapes.root","read")
data3 = TFile("shapes.root","read")
data4 = TFile("shapes.root","read")

dataHist1=data1.Get("electron_2018_750_900_prefit/data_obs")
dataHist1_2017=data2.Get("electron_2017_750_900_prefit/data_obs")
dataHist1_2016=data3.Get("electron_2016postVFP_750_900_prefit/data_obs")
dataHist1_2016pre=data4.Get("electron_2016postVFP_750_900_prefit/data_obs")

total_n_bins = dataHist1.GetXaxis().GetNbins()
data = TH1F("data","data",nb,array('d',Bin))

for i in range(1,total_n_bins+1):
    if(i < 100):              data.SetBinContent(i,dataHist1.GetBinContent(i) + dataHist1_2017.GetBinContent(i) + dataHist1_2016.GetBinContent(i) + dataHist1_2016pre.GetBinContent(i))
...
bkg_bin1_2016=shapes.Get("electron_2016postVFP_750_900_prefit/TotalProcs")
bkg_bin1_2016pre=shapes.Get("electron_2016postVFP_750_900_prefit/TotalProcs")
...
for i in range(1,total_n_bins+1):
    ...
    if(i < 10):         errorband.SetBinError(i,math.sqrt(dataHist1.GetBinError(i)**2+dataHist1_2017.GetBinError(i)**2+dataHist1_2016.GetBinError(i)**2+dataHist1_2016pre.GetBinError(i)**2)/(bkg_bin1.GetBinContent(i)+bkg_bin1_2017.GetBinContent(i)+bkg_bin1_2016.GetBinContent(i)+bkg_bin1_2016pre.GetBinContent(i)))
