...
string texts[]={"DATA","TTToSemiLeptonic","WJetsToLNu","ST","DYJetsToLL_M-50_HT","QCD_HT","Diboson","TTToSemiLeptonic2"};
...
//----obetener_toda_la_informacion_de_entrada--------??
    ...
    if(text == "TTToSemiLeptonic2"){ 
        chreco_ttbar_semi->Add("/nfs/dust/cms/user/hugobg/analysis/electron/uhh2.AnalysisModuleRunner.MC.TTToSemiLeptonic_2016preVFP.root/AnalysisTree");
    }else{
...
//--------Unfold----------??
       ...
       string selcuts_boosted_central_1 = "(Ele_pt > 55 && DeepttagN <= 1 && btagN >= 1 && M_tt > 500 && chi2 < 30 && Ak4_add_pt > 100 && angle_theta_opt > 0.25*3.1459 && angle_theta_opt < 0.60*3.1459)";
...
//        string systs[] = {"1"};
        ...
        for(const string &syst : systs){
            ...
            string selcuts_boosted_central_1 = "(Ele_pt > 55 && DeepttagN <= 1 && btagN >= 1 && M_tt > 500 && chi2 < 30 && Boost_Ak4_add_pt > 100 && angle_theta_opt > 0.25*3.1459 && angle_theta_opt < 0.60*3.1459 && Gen_angle_theta_opt > 0.25*3.1459 && Gen_angle_theta_opt < 0.60*3.1459)";
...
//           treereco_ttbar_semi->Project("Migration_Matrix_1","Gen_Boost_Ak4_add_pt:Boost_Ak4_add_pt",Form("%s*eventweight*%s",selcuts_boosted_central_1.c_str(),syst.c_str()));
         ...
         for(int k=1;k<=2;k++){
            ...
            TH2F *Migration_Matrix_1 = new TH2F("Migration_Matrix_1","",newrec,new_rec,binnum_gen,bins_gen);
            string selcuts_boosted_central_1 = "(Ele_pt > 55 && DeepttagN <= 1 && btagN >= 1 && M_tt > 500 && chi2 < 30 && Ak4_add_pt > 100 && angle_theta_opt > 0.25*3.1459 && angle_theta_opt < 0.60*3.1459 && Gen_angle_theta_opt > 0.25*3.1459 && Gen_angle_theta_opt < 0.60*3.1459)";
            treereco_ttbar_semi->Project("Migration_Matrix_1","Gen_Boost_top_E - Gen_Boost_antitop_E:Boost_top_E - Boost_antitop_E",Form("%s*eventweight*%s",selcuts_boosted_central_1.c_str(),syst.c_str()));
            TH1D* Ttbar_1_nominal = Migration_Matrix_1->ProjectionX("Ttbar_1_nominal", 1,1);
            TH1D* Ttbar_2_nominal = Migration_Matrix_1->ProjectionX("Ttbar_2_nominal", 2,2);
       ...
       } else if(text == "TTToSemiLeptonic2"){
        ...
        for(const string &syst : systs){
            ...
            Back_1->Write(Form("%s_nonfiducial_%s",tt.c_str(),sys.c_str()));
            delete Back_1;
