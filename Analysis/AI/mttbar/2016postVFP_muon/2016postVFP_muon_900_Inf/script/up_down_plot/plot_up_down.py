...
fin = TFile('../Input_undfolding_data_.root', 'open')
nominalhist = {}
nominalhistDraw = {}
systvarhist = {}
systvarhistDraw = {}
...
for samp in samplelist:
    for syst in systematic_direction:
        ...
        canvas_Bkg[samp+syst] = TCanvas("SystVariation_"+samp+syst,"SystVariation_"+samp+syst,1200,1400)
        legend = TLegend(.7,.70,.9,.9)
        gStyle.SetOptStat(0)
        gStyle.SetOptTitle(0)

        #Upper plot will be in pad1
        pad1[samp+syst] = TPad("pad1", "pad1", 0.05, 0.3, 1., 0.95);
        ...
        systvarhistDraw[samp+syst+'Up'].GetYaxis().SetLabelSize(0.02)
        systvarhistDraw[samp+syst+'Up'].GetYaxis().SetTitle("Events")
#        systvarhistDraw[samp+syst+'Up'].GetXaxis().SetLabelSize(0.)
        ...
        print(systvarhistDraw[samp+syst+'Up'].Integral())
        print(nominalhistDraw[samp+syst].Integral())
        print(systvarhistDraw[samp+syst+'Down'].Integral())
        ...
        latex.DrawLatex(-1999.,a*1.5,"Work in progress")

        latex2 = ROOT.TLatex()
        latex2.SetTextSize(0.045)
        latex2.SetTextAlign(11)
        latex2.DrawLatex(500,a*1.5,"16.8 fb^{-1} (13 TeV)")

        canvas_Bkg[samp+syst].cd();          # Go back to the main canvas before defining pad2
        pad2[samp+syst] = TPad("pad2", "pad2", 0.05, 0.15, 1, 0.365);
        ...
        systvarhistRatio[samp+syst+'__plus__ratio'].GetYaxis().SetLabelSize(0.06)
        systvarhistRatio[samp+syst+'__plus__ratio'].GetYaxis().SetTitle("")
        systvarhistRatio[samp+syst+'__plus__ratio'].GetXaxis().SetTitleSize(0.059)
        systvarhistRatio[samp+syst+'__plus__ratio'].GetXaxis().SetLabelSize(0.06)
        ...
        systvarhistRatio[samp+syst+'__minus__ratio'].GetYaxis().SetTitleSize(6)
        pad2[samp+syst].Update()
        canvas_Bkg[samp+syst].Update()
